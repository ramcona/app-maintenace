...
# Initialize logger for better logging within Python functions
...
def _get_active_archive_configs(**kwargs):
    ...
    try:
        ...
        # Use quoted names for case-sensitive columns
        columns_to_select = []
        column_names = []
        ...
        # Optional columns (may exist with different naming conventions)
        optional_cols = [
            ('createdAt', 'created_at'),  # Try both camelCase and snake_case
...
def _ensure_schema_and_table_exists(single_config: dict, **kwargs):
    ...
    try:
        ...
        # Note: Schema and table names cannot be parameterized in PostgreSQL
...
with DAG(
    ...
    # Dynamic Task Mapping - Fixed version
